...
# --- 2. Горизонтальна частина літери ---
...
# --- Letter ---
o_outer = 2.5 - sp.sqrt(((x - 6.5) / 1) ** 2 + ((y) / 1.2) ** 2)
o_inner = 2.5 - sp.sqrt(((x - 6.5) / 0.8) ** 2 + ((y) / 1) ** 2)
o_shape = dr(o_outer, o_inner)

gamma_shape = ur(gamma_shape, o_shape)
